...
t=t(t.Year==2019,:);
% t=t(ismember(t.experimenter,{'Qihan'}),:);
t=t(ismember(t.experimenter,{'darshan','Darshan'}),:);
% t=sortrows(t,'block');
t(:,{'dataFilename' 'Year' 'trials' 'experimenter' 'observer' 'block' 'blocksDesired'  'eccentricityXYDeg' 'conditionName'})
% return
...
t(:,{'dataFilename' 'experimenter' 'observer' 's' 'block'})
    return
...
% tt=t(ismember(t.observer,{'Sam Colman'}),:);
tt=t;
cd(dataFolder);
