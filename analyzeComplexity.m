...
if printFilenames
    ...
    switch experiment
        case 'runCrowdingSurvey'
            disp(t(:,{'observer' 'targetFont'  'targetDeg' ...
                'thresholdParameter'  ...
                'eccentricityXYDeg' 'spacingDeg' ...
                'dataFilename' ...
                }));
    end
end
sizeRows=ismember(t.thresholdParameter,{'size'});
t=t(sizeRows,{'targetFont' 'targetDeg' 'observer' });
tt=sortrows(t,{'targetFont','observer'});
disp(tt)
writetable(tt,'ComplexitySize.xlsx');
return
...
%% PLOT HISTOGRAMS (ACROSS OBSERVERS & HEMISPHERES) OF THREE KINDS OF THRESHOLD. AT ±10,0 DEG.
figure;
width=25;
height=50;
...
%             name=sprintf('%s, mean %.1f%c%.1f',name,m,plusMinus,sd);
    ...
    ylabel('Count');
    xlabel([parameter]);
    title(sprintf('Histogram of %d hemispheres at (%c%.0f,%.0f) deg',...
        length(x),plusMinus,abs(t{i(1),'eccentricityXYDeg'})));
    ax=gca;
    ax.FontSize=12;
