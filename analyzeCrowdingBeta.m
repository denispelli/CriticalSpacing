...
experiment='CrowdingBeta'; % And CrowdingSurvey2.
printFilenames=false;
makePlotLinear=false;
mainPath=fileparts(mfilename('fullpath')); % Takes 0.1 s.
addpath(fullfile(mainPath,'lib')); % Folder in same directory as this M file.
QuestPlusPath=fullfile(fileparts(mainPath),'mQUESTPlus');
...
addpath(fullfile(QuestPlusPath,'mathworkscentral','allcomb'));
addpath(fullfile(QuestPlusPath,'mathworkscentral','von_mises_cdf'));
...
vars={'experiment' 'condition' 'conditionName' 'dataFilename' ...
    ...
    'trials' 'trialsDesired' 'trialData' 'responseCount' ...
    'thresholdParameter' ...
    ...
    'simulateObserver' 'simulatedLogThreshold' 'spacings' ...
    'q' 'useQuest' 'alphabet' 'questMean' 'questSD' ...
    };
...
ok=logical([]);
for oi=1:length(oo)
    oo(oi).trials=length(oo(1).trialData);
    switch oo(oi).task
        case 'identify'
...
%% For each observer produce new condition that combines all data.
observers=unique({oo.observer});
for observerCell=observers
    ii=find(ismember({oo.observer},observerCell));
    jj=[];
    for oi=ii
        if abs(oo(oi).eccentricityXYDeg(1))==5
            jj(end+1)=oi;
        end
    end
    ii=jj;
    if length(ii)>1
        o=oo(ii(1));
        if isfield(o,'trialData')
            % We're not updating it, so discard it.
            o.trialData=[];
        end
        for oi=ii(2:end)
            o.trials=o.trials+oo(oi).trials;
            o.trialsDesired=o.trialsDesired+oo(oi).trialsDesired;
            t=QuestTrials(oo(oi).q);
            for i=1:length(t.intensity)
                intensity=t.intensity(i);
                for j=1:t.responses(1,i)
                    oo(oi).q=QuestUpdate(oo(oi).q,intensity,0);
                end
                for j=1:t.responses(2,i)
                    oo(oi).q=QuestUpdate(oo(oi).q,intensity,1);
                end
            end
        end
        oo(end+1)=o;
...
for oi=1:length(oo)
    oo(oi).questPlusEnable=true;
    oo(oi).questPlusPlot=true;
    oo(oi).questPlusSteepnesses=1:0.2:4;
    oo(oi).questPlusGuessingRates=1/length(oo(oi).alphabet);
    oo(oi).questPlusLapseRates=10.^[-3:0.5:-1];
    oo(oi).questPlusPrint=true;
    oo(oi).guess=1/length(oo(oi).alphabet);
    oo(oi).lapse=nan;
    oo(oi).steepness=nan;
    oo(oi).noiseSD=0; % CriticalSpacing never has noise.
    % ALL INTENSITIES ACTUALLY TESTED ARE APPENDED TO OUR QUESTPLUS
    % SAMPLING PLAN.
    t=QuestTrials(oo(oi).q);
    intensities=unique(t.intensity);
    oo(oi).questPlusLogIntensities=unique([-3:.2:0.5 intensities]);
end
...
% Assume we are given a huge oo struct, and each row has one condition, and
