function machine=IdentifyComputer2(screens,modifier1,modifier2)
% machine=IdentifyComputer2([screens,modifier1,modifier2]);
...
%% INPUT ARGUMENTS:
% Use the "screens" argument scalar or array to specify the screen numbers
% you want to test. The main screen is 0, the next is 1, and so on. Default
% is all screens. This routine is quick if "screens" is empty [] or
...
% take 30 s. Passing an empty "screens" argument skips opening a window, at
...
if exist('PsychtoolboxVersion','file')
    ...
    % MATLAB recommends "contains", but it's not available in Octave.
...
if nargin<1
    screens=machine.screens;
end
if any(~ismember(screens,machine.screens))
    error('One or more of the specified screens is illegal.');
end
machine.screen=num2cell(screens);
machine.size=cell(size(screens));
machine.nativeSize=cell(size(screens));
machine.mm=cell(size(screens));
if exist('PsychtoolboxVersion','file') && ~isempty(screens)
    for iScreen=1:length(screens)
...
machine.openGLRenderer=cell(size(screens));
machine.openGLVendor=cell(size(screens));
machine.openGLVersion=cell(size(screens));
for iScreen=1:length(screens)
...
if exist('PsychtoolboxVersion','file') && ~isempty(screens)
    for iScreen=1:length(screens)
        screen=screens(iScreen);
        % For each screen we use an open window, if available, or open one.
        window=[];
        isNewWindow=false;
        % Try to find a window already open on the specified screen.
        windows=Screen('Windows');
        for i=1:length(windows)
            if screen==Screen('WindowScreenNumber',windows(i))
        ...
        if isempty(window)
            ...
            if islinux
                % This is safer, because one user reported a fatal error
                % when attempting to open a less-than-full-screen window
                % under Linux.
...
end % if exist('PsychtoolboxVersion','file') && ~isempty(screens)
if exist('PsychtoolboxVersion','file')
    try
        if ~verbose
