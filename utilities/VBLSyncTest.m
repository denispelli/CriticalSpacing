...
%
% Tests syncing of Psychtoolbox to the vertical retrace (VBL) and
% demonstrates how to implement the old Screen('WaitBlanking') behaviour
% with Screen('Flip')...
%
...
% performs a monitor calibration timing loop to estimate the real monitor
% refresh interval (aka IFI): While the formula ifi = 1.0 /
% Screen('NominalFramerate') will return an ifi that is close to the real
% IFI, it will be a little bit off from the real value. The reason is
% unavoidable jitter in the manufacturing of graphics cards internal clock
% circuits, as well as some drift and jitter caused by instabilities and
% change in the power supply and operating temperature of your machine. To
% be on the safe side, we use a timing-loop to compute the real IFI as an
% average of the IFI's of a number of consecutive monitor refresh
% intervals.
%
...
%
% This would be roughly equivalent to the following snippet of code in the
% old macOS9-PTB:
% ...
...
%
%
% loadjitter = Simulated load with a random duration between 0 ms and
% loadjitter monitor refresh intervals: We wait for the specified amount of
% time to simulate the execution of other Matlab-code, e.g., KbChecks,
% GetMouse, Matlab calculations ...
%
%
%
% clearmode = Change the behaviour of Flip after flipping.
% clearmode = 0 will clear your stimulus drawing surface to background
% color after flip. This is the behaviour as found in PTB 1.0.50. After
% Flip you start with an empty image and can draw a completely new stim.
% 
% clearmode = 1 will not clear after a flip, but keep the contents of your
% stimulus image after the Flip: This allows you to incrementally
% update/draw stimuli.
%
% clearmode = 2 will neither clear nor keep the drawing surface after Flip,
% but leave the cleanup work to you. To be precise: The drawing surface
% will contain the stimulus image that was *just shown* on the screen.
% Think of Flip as if it would flip the front- and back-side of a sheet of
% paper: The current front side shows the stim to your subject, the current
% back side is where you draw. clearmode 2 will allow you to update the
% back side, which was the front side before the flip happened! This mode
% is useful if you want to save about 0.5-2 ms of time needed for mode 1 or
% 2 if you draw stimuli on very tight deadlines.
%
%
% stereo = Test timing of display of stereoscopic stimuli.
% stereo = 0 will show you a standard monoscopic display.
% stereo = 1 will use the OS-X stereo output facilities to show
% stereoscopic stimuli: OS-X will quickly alternate between two images at
% each monitor refresh, one for the left-eye, one for the right-eye, while
% generating proper control signals for LCD shutter glasses. This should
% work with MacOS-X compatible stereo display hardware, e.g., CrystalEyes
% shutter glasses.
%
%
% flushpipe = Mark end of drawing commands to improve presentation timing.
% PTB knows a new command Screen('DrawingFinished') which, when properly
% used, will give PTB hints on how to optimize drawing of stimuli: This
% allows to draw more complex stimuli at higher monitor refesh intervals
% with reliable presentation timing. flushpipe enables/disables use of this
% new command.
%
...
% synchronous = 1 Wait for completion - Useful for benchmarking and
% debugging, but degrades performance significantly in real experiments.
%
%
...
% Clear the framebuffer after flip, don't use stereo output, but use the
% new "DrawingFinished" command.
%
% VBLSyncTest(100, 10, 6, 1, 0, 1, 0) -- Render 100 frames,
% flip only every 10th monitor refresh interval ("WaitBlanking" for 10
% refresh intervals), pause for 6 frame durations after each flip. Don't
% clear the framebuffer after flip, don't use stereo output, but use the
% new "DrawingFinished" command.
%
...
% flip only every 10th monitor refresh interval ("WaitBlanking" for 10
% refresh intervals), pause for 6 frame durations after each flip. Don't
% clear the framebuffer after flip, use stereo output via "frame-sequential
% stereo", use the new "DrawingFinished" command.
%
%
%
% THE PLOTS:
...
% jitter of less than +/- 1 ms indicates proper stimulus presentation
% timing - no skipped frames.
%
%
...
% your computer is either pretty slow, or connected to a flat-panel. Lots
% of randomly distributed values between 0 and 1024 would indicate sync
% trouble.
%
%
...
% deadline has been met. While the sign of this value is useful for
% assessing timing, the value itself is only meaningful for people who can
% read and fully understand the C source code and logic of 'Flips'
% implementation...
%
% Figure 4: Shows the difference (in milliseconds) between estimated start
% of VBL and return of the Flip command to Matlab. This is some indication
% of the processing overhead of OpenGL, the Operating system and
...
% stimulus onset (aka end of vertical retrace, scanning beam starts at top
% of screen) and the end of Flip. This is the crucial value, if you want to
% sync something like sound-playback, triggering of some data-acquisition
% device (fMRI, MEG, EEG, ...) to stimulus onset. It should give you a
% feeling of how well you can sync. It's possible that negative values are
% reported on fast machines - Flip returns ahead of time (while monitor is
% still in retrace state). This is fine because your Matlab-Code for
% triggering something will add additional delays... Values should be below
% 1-2 milliseconds on reasonably modern and correctly configured hardware.
%
% Figure 6: Is only displayed when flag 'synchronous=1' This figure shows
% the total accumulated time for all drawing commands from the last Flip to
% the DrawingFinished command. It allows you to get a feeling on how hard
% the graphics hardware has to work for drawing your stim - and if it is
...
% the best possible presentation timing on PTB-macOS.
%

% 05/09/05 Mario Kleiner (mario.kleiner.de@gmail.com) Wrote it.
% 22/10/19 denis.pelli@nyu.edu Automatically save the plots to disk.

% The plots will be saved in the current folder. You can use "cd" to select
% whatever folder you like. The plot files will be PNG files and each
% filenames will begin with "VBLSyncTest-".
...
try
    ...
    fprintf('All plots have been saved as files in folder: ''%s''.\n',pwd);
    fprintf('<strong>Have a look at the plots for more details.</strong>\n');
...
%% SAVE PLOT TO DISK
function SavePlotToDisk(h)
% SavePlotToDisk(h);
% Receives a figure, renames the title bar to identify the computer
% and OS, and saves it to disk with that name, in the current folder.
machine=IdentifyComputer;
figureTitle=['VBLSyncTest-' num2str(h.Number) '-' machine.model '-' machine.system ...
