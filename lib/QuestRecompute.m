...
% false.)
%
% QuestCreate saves in struct q the parameters for a Weibull psychometric 
% function:
...
% 04/7/20   dgp  Initialize q.intensity and q.response with nans instead of
%                zeroes.
...
if ~isfield(q,'intensity') || ~isfield(q,'response')
    ...
    q.trialCount = 0;
    q.intensity=nan(1,10000);
    q.response=nan(1,10000);
end
