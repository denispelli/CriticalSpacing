function [string,terminatorChar] = GetEchoStringNYU(windowPtr, msg, x, y, textColor, bgColor, useKbCheck, varargin)
% [string,terminatorChar]=GetEchoStringNYU(window,msg,x,y,[textColor],[bgColor],[useKbCheck=0],[deviceIndex],[untilTime=inf],[KbCheck args...]);
% NYU VERSION: This is our customized version of GetEchoString. 1. We omit
% the alphablending call, which causes compatability problems on Windows.
% 2. Enhanced to accept the ESCAPE key as yet another terminator, like
% Ctl-C, Return, and Enter. 3. Enhanced to accept a second, optional,
% output argument that returns the ASCII code of the terminator. 4. To make
% this work, we also enhanced GetKbCharNYU to pass the ESCAPE key.
% denis.pelli@nyu.edu February 14, 2016.
%
% ORIGINAL GetEchoString DOCUMENTATION:
% 
...
if ~isempty(bgColor)
   % NYU version: commented out because it causes trouble on Window,
   % resulting in an annoying box around every letter, even if you don't
   % use the delete key.
