...
for i=1:length(letters)
    if o.useMATLABFontRendering
        ...
        text(0,256, letters(i), 'Units', 'pixels', 'FontName', 'Sloan','FontUnits', 'pixels', 'FontSize', 512);
        box off
        axis off
        set(gca,'XTick',[]) % Remove the ticks in the x axis!
        set(gca,'YTick',[]) % Remove the ticks in the y axis
        F = getframe(gcf);
        letterImage= frame2im(F);
        close;
        letterImage=letterImage(:,:,2);
        savedAlphabet(ia).bounds{i}=ImageBounds(letterImage,letterImage(1,1));
        savedAlphabet(ia).images{i}=letterImage;
    Screen('PutImage',window,letterImage);
    Screen('Flip',window);
    WaitSecs(0.5);
    else
...
if o.useMATLABFontRendering
    ...
    for i=1:length(letters)
        savedAlphabet(ia).images{i}=savedAlphabet(ia).images{i}(1+b(1):b(3),1+b(2):b(4));
    end
